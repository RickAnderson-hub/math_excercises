 ...
 *
 * <ul>
 *   <li>Requested count honored.
 *   <li>Operand/result bounds respected.
 *   <li>Operator set restricted to + / - in default mode.
 *   <li>No zero+zero equations; non-negative invariants enforced.
 *   <li>Validation for lower-than-minimum limit.
 * </ul>
 */
class GenerateServiceTests {
	...
	void generatesExercisesWithinLimit() {
		...
		equations.forEach(
				equation -> {
					assertTrue(equation.result() <= limit);
					assertTrue(equation.firstNumber() <= limit);
					assertTrue(equation.secondNumber() <= limit);
				});
	}
	...
	void generatesExercisesWithValidOperators() {
		...
		List<Equation> equations = generateService.generateExercises(limit, 20);
		equations.forEach(
				equation -> assertTrue(equation.operator() == '+' || equation.operator() == '-'));
	}
	...
	void generatesPositiveResultsForAddition() {
		...
		equations.stream()
				.filter(equation -> equation.operator() == '+')
				.forEach(equation -> assertTrue(equation.result() >= 1));
	}
	...
	void generatesNonNegativeNumbers() {
		...
		equations.forEach(
				equation -> {
					assertTrue(equation.firstNumber() >= 0);
					assertTrue(equation.secondNumber() >= 0);
					assertFalse(equation.firstNumber() == 0 && equation.secondNumber() == 0);
					assertTrue(equation.result() >= 0);
				});
	}
	...
	void handlesLowerLimit() {
		int limit = 9;
		assertThrowsExactly(
				IllegalArgumentException.class, () -> generateService.generateExercises(limit, 1));
	}
}
